...
- **音源**：当前只有酷我（`lx-source.js`）在词典笔上实测可用（搜索、320k 完整版、歌词、下载全通）。
  kg/mg/wy/tx 数据层可编译可调用，但部分接口受平台签名/风控限制，设备上可用性不稳定；
  sixyin / ikun / flower / grass / huibq 等第三方音源已逐一实测：接口拒绝 curl、API 停服或域名失效，**不提供**。
- **自动连播依赖宿主**：播完自动切歌由宿主播放器的 `onSoundEnd → onClickedNext` 机制驱动，
  插件只负责推进队列；若宿主行为变化，连播可能失效（8 秒防误触与会话号校验已尽量兼容）。
- **播放 URL 依赖用户音源脚本**：音源脚本需自行提供有效 `musicUrl`；示例脚本仅支持 kw。
...
- **旧插件**：设备上历史版本 `com.penmods.music`（PenMods-Music）建议在插件管理器禁用，
  否则每次启动会尝试抢占同一批宿主钩子失败并产生日志噪音。
